...
const task_arr = [
    ['Finish 4261 project', 'Complete a usable todo list app for android and web', ['Finish backend', 'Finish frontend web', 'Finish frontend Android']],
...
function createTasks() {
    ...
    for (let i = 0; i < task_arr.length; i++) {            
        const subtasks = task_arr[i][2].map((val) => ({ name: val, isCompleted: false }))
        const newTask = {
            t_id: chance.guid(),
            name: task_arr[i][0],
